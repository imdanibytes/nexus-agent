...
import type { TimingSpan } from "../../stores/threadStore";
...
function getBarBorder(name: string): string {
  if (name === "turn")
    return "border border-default-200/50 rounded-sm";
  if (name.startsWith("round:"))
    return "border border-default-200/20 rounded-sm";
  if (name === "tool_execution")
    return "border border-dashed border-emerald-400/40 dark:border-emerald-500/30 rounded-sm";
  return "";
}
...
function getGridlines(totalMs: number): number[] {
  ...
  const interval =
    intervals.find((i) => i >= ideal) ?? intervals[intervals.length - 1];
  const lines: number[] = [];
...
export const TimingWaterfall: FC<{ spans: TimingSpan[] }> = ({ spans }) => {
  ...
  const { rows, totalMs, gridlines } = useMemo(() => {
    ...
    const total = turnSpan
      ? turnSpan.span.endMs
      : Math.max(...spans.map((s) => s.endMs), 1);
  ...
  return (
    <div className="flex flex-col text-xs font-mono">
      ...
      <div className="flex items-center border-b border-default-200/50 pb-1 mb-1 text-default-500">
        ...
        <div className="w-20 shrink-0 px-1 text-right font-semibold">
          Duration
        </div>
      </div>
              ...
              >
                <span
                  className={[
                    ...
                    getBarColor(span.name).replace(
                      "bg-transparent",
                      "bg-default-200/50",
                    ),
                  ].join(" ")}
                />
                <span className={isContainer ? "text-default-500" : ""}>
