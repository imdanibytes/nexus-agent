use nexus_core::tasks::{derive_mode, Plan, Task, TaskState, TaskStatus};
...
/// Dispatch a task tool call against the given state. Returns (content_json, is_error).
///
/// The caller is responsible for locking the store, persisting, and emitting events.
pub fn execute(
    tool_name: &str,
    ...
    let result = match tool_name {
        "task_create_plan" => create_plan(args, state),
        "task_approve_plan" => approve_plan(args, state),
        "task_create" => create_task(args, state),
        "task_update" => update_task(args, state),
        "task_list" => list_tasks(state),
...
fn create_plan(
    ...
    let plan = Plan {
        ...
        conversation_id: String::new(), // set by caller if needed
    ...
    if state.plan.is_some() {
        return Err(
            "A plan already exists for this conversation. Update or clear it first.".into(),
        );
    }
...
fn list_tasks(state: &TaskState) -> Result<serde_json::Value, String> {
